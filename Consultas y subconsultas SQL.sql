/*La empresa El Desván, que se dedica a la rama textil ha decidido informatizar su gestión de nóminas. Para ello BK programación 
desarrollará para ellos la base de datos.
El gerente le ha explicado cómo funciona la gestión de nóminas y Juan, que será quien se encargue de crear el modelo, las tablas y 
las consultas, ha recogido la siguiente información:
o	A cada empleado se le entrega un justificante de nómina al mes. De cada empleado registraremos su código de empleado, nombre, 
apellidos, número de hijos, cuenta corriente y porcentaje de retención para Hacienda.
...
o	La distinción entre dos justificantes de nómina se hace, además de mediante el código de empleado, mediante el ejercicio fiscal y 
número de mes al que pertenece.
o	Cada justificante de nómina consta de varias líneas y cada línea se identifica por un número de línea del correspondiente 
justificante. Una línea puede corresponder a un ingreso o a un descuento. En ambos casos se recoge la cantidad (positiva o negativa).
...
--5. Código del empleado, Mes y ejercicio de los justificantes de nómina pertenecientes los empleado cuyos códigos 
-- sean 1 , 21, 13 o 4 ordenado por código.
...
--10. Número de hijos y número de empleados agrupados por hijos, mostrando sólo los grupos cuyo número de empleados sea 
--mayor que 1. Es decir, mostrar cuántos empleados hay que tengan 1 hijo, 2 hijos,…
...
SELECT T.COD_DEP, COUNT(E.CODIGO) "NºTRABAJADORES" 
...
/*15.  Nombre del empleado, mes y ejercicio de sus justificantes de nómina, número de línea y cantidad de las líneas de los 
justificantes para el empleado cuyo código=1.*/
SELECT E.NOMBRE, L.MES, L.EJERCICIO, L.NUMERO, L.CANTIDAD 
FROM LINEAS L, EMPLEADOS E
WHERE L.COD_EMP=E.CODIGO AND E.CODIGO=1;


/*16.  Nombre del empleado, mes y ejercicio de sus justificantes de nómina para los empleados que han trabajado en el 
...
/*23.  Nombre de los empleados, nombre de los departamentos en los que ha trabajado y función en mayúsculas que ha realizado en cada
departamento.*/
...
Sólo tendremos en cuenta aquellos departamentos que tengan más de 3 empleados y los años por departamento en los que la suma de los
ingresos de los empleados sea mayor de 60.000€.*/
...
/*29.  Visualizaremos nombre del departamento, el nº de empleados que tiene, suma de los ingresos y descuentos de todos sus 
empleados del departamento o departamentos con menor número de empleados.*/
...
/*30.  Nombre, media de los  ingresos de  los empleados que tengan más de 2 hijos y que pertenezcan al departamento o departamentos 
